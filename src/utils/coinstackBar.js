...
export function CoinStackBar(o) {
  ...
  this.containerheight =
    (this.coinimgheight + this.maxstackheight * this.coinheight) *
    this.stacksize;
  ...
  this.shadow.style.background =
    "-webkit-radial-gradient(ellipse closest-side, black, rgba(0,0,0,0))";
  this.shadow.style.background =
    "radial-gradient(ellipse closest-side, black, rgba(0,0,0,0))";
...
/*CoinStackBar.setDimensions: Method to change the width or height of the CoinStackBar.
            ...
            */
CoinStackBar.prototype.setDimensions = function(
  containerwidth,
  containerheight
) {
  var picksmaller = "";
...
CoinStackBar.prototype.setValue = function(value, newmin, newmax) {
  ...
  if (clamped == this.value) {
    //Do nothing if there's no change in value
    return false;
  }

  //Otherwise: Calculate the amount of coins to show for the current value.
  // NOTE: The follow two lines of code have been changed by Cory House so the value passed in determines the number of coins displayed. (since we only want to display a single type of coin and aren't interested in calcuating coin values)
  var coins = value;
  //Also calculate the amount of coins that were already in the stack.
  var oldcoins = this.value;

  //If we want to reseed, change the seed.
  if (
    this.reseed == "always" ||
    (coins == 0 && oldcoins != 0 && this.reseed == "onzero")
  ) {
...
CoinStackBar.prototype.createNewCoin = function(i, durationoffset) {
  ...
  if (Object.prototype.toString.call(this.coinimgsrc) === "[object Array]") {
    var index = Math.floor(
      Math.abs(this.rand(i + 1, this.seed) * this.coinimgsrc.length)
    );
  ...
  var coinypos =
    i * (this.coinheight * this.stacksize) +
    this.rand(i, this.seed) * (this.yoffset * this.stacksize);
  ...
  if (this.coins.length > 0) {
    starttime = Math.max(
      this.coins[this.coins.length - 1].starttime +
        this.coineffectduration * 0.3,
      starttime
    );
  }
...
CoinStackBar.prototype.fadeOutCoin = function(i, durationoffset) {
  ...
  if (this.coins[i] && this.coins[i].status != "fadeout") {
    this.coins[i].starttime =
      Date.now() + durationoffset * this.coins[i].duration * 0.3;
...
CoinStackBar.prototype.update = function() {
  ...
  if (this.coins) {
    ...
    this.coins.forEach(function(coin, key, CoinStackBar) {
      ...
      var animframe =
        1.0 - (coin.starttime + coin.duration - Date.now()) / coin.duration;
      animframe = Math.max(Math.min(1, animframe), 0);

      //Fade coin in.
      if (coin.status == "fadein") {
        coin.element.style.bottom =
          coin.ypos + coin.ystart * (1.0 - animframe) * this.stacksize + "px";
      ...
      if (coin.status == "fadeout") {
        ...
        if (this.coins[key - 1]) {
          coin.element.style.opacity = Math.min(
            coin.element.style.opacity,
            CoinStackBar[key - 1].element.style.opacity
          );
        }
      }
    }, this);
